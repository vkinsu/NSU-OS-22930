...
void terminal_init(terminal_d *terminal_d){
    ...
    terminal_d->cl = 40;
    terminal_d->data = (char*)calloc(terminal_d->cl, 1);
    terminal_d->size = terminal_d->cl;
    terminal_d->ln = 0;
...
void terminal_write(terminal_d *terminal_d, char ch){
    if(!isprint(ch) && ch != '\n') return;
    if (ch == '\n')
        terminal_d->last_end = terminal_d->ln;
    if (terminal_d->ln > 0 && terminal_d->ln + 1 - terminal_d->last_end > terminal_d->cl) {
        char next_line = '\n';
        write(STDOUT_FILENO, &next_line, sizeof(char));
        terminal_d->last_end = terminal_d->ln;
    }
    while (terminal_d->ln + 1 >= terminal_d->size){
        terminal_d->data = (char*)realloc(terminal_d->data, terminal_d->size + terminal_d->cl);
        terminal_d->size += terminal_d->cl;
    }
    write(STDOUT_FILENO, &ch, sizeof(char));
    terminal_d->data[terminal_d->ln++] = ch;
}

void terminal_kill(terminal_d *terminal_d){
    if(terminal_d->ln == 0) return;
    while(terminal_d->ln > 0 && terminal_d->data[terminal_d->ln] != '\n'){
        terminal_d->ln--;
...
void terminal_erase_word(terminal_d *terminal_d){
    if(terminal_d->ln == 0) return;
    while(terminal_d->ln > 1 && isspace(terminal_d->data[terminal_d->ln - 1])){
        terminal_d->ln--;
        write(STDOUT_FILENO, "\b \b", sizeof(char) * 3);
    }
    while(terminal_d->ln > 0 && !isspace(terminal_d->data[terminal_d->ln - 1])){
        terminal_d->ln--;
        write(STDOUT_FILENO, "\b \b", sizeof(char) * 3);
    }
}
